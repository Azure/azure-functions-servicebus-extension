    ...
    internal sealed class ServiceBusListener : IListener
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                {
                    _receiver = _messagingProvider.CreateMessageReceiver(_serviceBusAccount.EntityPath, _serviceBusAccount.ConnectionString);
                    StartMultipleMessageProcessing(_cancellationTokenSource.Token);
                }
            }
            _started = true;
        ...
        internal void StartMultipleMessageProcessing(CancellationToken cancellationToken)
        {
                Task.Run(async () =>
                {
                    while (_receiver != null && !_receiver.IsClosedOrClosing && !cancellationToken.IsCancellationRequested)
                    {
                        try
                        {
                            IList<Message> messages = null;
                            messages = await _receiver.ReceiveAsync(_maxMessageCount);
                            ...
                            if (messages != null && messages.Count > 0)
                            {
                                ...
                                IEnumerable<string> allLockTokens = messagesArray.Select(x => x.SystemProperties.LockToken);
                                // If function execution is failed abandon all messages 
                                if (!result.Succeeded)
                                {
                                    // No batch abandon available, causing slowness when processing fails
                                    foreach (string lockToken in allLockTokens)
                                    {
                                        await _receiver.AbandonAsync(lockToken);
                                    }
                                }
                                else
                                {
                                    // Complete all messages as ReceiveAsync uses PeekLook by deault
                                    await _receiver.CompleteAsync(allLockTokens);
                                }
                            }
                        }
                        catch (Exception ex)
                        {
                            _serviceBusOptions.ExceptionHandler.Invoke(new ExceptionReceivedEventArgs(ex,
                                "BatchReceive",
                                _receiver.ServiceBusConnection.Endpoint.ToString(),
                                _receiver.Path,
                                _receiver.ClientId));
                        }
                    }
